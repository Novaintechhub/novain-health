...
export default function PatientRegistration() {
  ...
  const onSubmit = async (data: RegistrationInput) => {
    try {
      ...
      if (!response.ok) {
         const errorMessage = result.error?.email?._errors[0] || result.error || 'An unexpected error occurred';
      ...
      router.push(`/verify-email?role=patient`);
          ...
          <div className="w-full lg:w-1/2 flex flex-col items-center justify-center p-4 sm:p-8 overflow-y-auto bg-white">
            <div className="w-full max-w-lg">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-2">
                ...
                <Link href="/doctor-registration" className="text-sm text-cyan-500 hover:underline">Not a Patient?</Link>
              </div>

              <div className="space-y-3">
                <Button variant="outline" className="w-full justify-center" onClick={() => signInWithGoogle().then(u => u && router.push('/patients/dashboard'))}>
                  <GoogleIcon className="mr-2 h-5 w-5" />
                  Continue with Google
                </Button>
                <Button variant="outline" className="w-full justify-center" onClick={() => signInWithApple().then(u => u && router.push('/patients/dashboard'))}>
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <FormField control={form.control} name="password" render={({ field }) => (
                      <FormItem><FormControl><Input type="password" placeholder="Input password" {...field} /></FormControl><FormMessage /></FormItem>
                    )}/>
                    ...
                    <FormField control={form.control} name="stateOfResidence" render={({ field }) => (
                      <FormItem>
                        <Select onValueChange={handleStateChange} defaultValue={field.value}>
                          <FormControl><SelectTrigger><SelectValue placeholder="State of Residence" /></SelectTrigger></FormControl>
                          <SelectContent>{nigerianStates.map(state => (<SelectItem key={state.name} value={state.name}>{state.name}</SelectItem>))}</SelectContent>
                        </Select>
                        <FormMessage />
                      </FormItem>
                    )}/>
                    <FormField control={form.control} name="lga" render={({ field }) => (
                      <FormItem>
                        <Select onValueChange={field.onChange} value={field.value} defaultValue={field.value} disabled={lgas.length === 0}>
                          <FormControl><SelectTrigger><SelectValue placeholder="Local Government of Residence" /></SelectTrigger></FormControl>
                    ...
                    <FormField control={form.control} name="language" render={({ field }) => (
                      <FormItem>
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                          ...
                          <SelectContent>{nigerianLanguages.map(lang => (<SelectItem key={lang} value={lang}>{lang}</SelectItem>))}</SelectContent>
                        </Select>
                        <FormMessage />
                      </FormItem>
                    )}/>
