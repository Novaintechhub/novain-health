  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-bold mt-2">Reschedule Appointment</h1>
      </div>
      <Card>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {availableSlots.length > 0 ? (
                    availableSlots.map((slot: {start: string, end: string}, index: number) => (
                        <Button
                          key={index}
                          variant={selectedTime === `${formatTo12Hour(slot.start)} - ${formatTo12Hour(slot.end)}` ? 'default' : 'outline'}
                          onClick={() => setSelectedTime(`${formatTo12Hour(slot.start)} - ${formatTo12Hour(slot.end)}`)}
                          className={`rounded-md ${selectedTime === `${formatTo12Hour(slot.start)} - ${formatTo12Hour(slot.end)}` ? 'bg-cyan-500 text-white' : ''}`}
                        >
